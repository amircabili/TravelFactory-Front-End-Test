...
import { PopupAlertComponent } from 'src/app/components/popup/popup-alert/popup-alert.component';
...
export class GlobalFunctionsService {
  ...
  openAlertModal(data) {
    const promise = new Promise((resolve, reject) => {
      ...
      dialogConfig.maxWidth = '650px';
      dialogConfig.data = data;
      const modalDialog = this.matDialog.open(PopupAlertComponent, dialogConfig);
      modalDialog.afterClosed().subscribe(result => {
        console.log('The dialog was closed ------>' + result);
        if (result) {
          resolve(result);
  ...
  getEmptySearchParms(){
    return {Title : '', Company: '',imdbID: '',Type: '',Poster: ''}
  }
